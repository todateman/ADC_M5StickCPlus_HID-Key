...
BleKeyboard bleKeyboard("M5STICK-C PLUS");  //デバイス名
...
void setup(void) {
    ...
    Wire.begin(0,26);
    M5.Lcd.setRotation(1);  // Rotating display.  旋转显示屏
    bleKeyboard.begin();
...
//        int16_t result = ads.Measure_Differential() * 0.41 - 0.1503;
        double result = map(ads.Measure_Differential(), 0, 3832, 0, 1571) ;
        M5.Lcd.fillScreen(BLACK);
        char data[20] = {0};
        sprintf(data, "%.1lf", result/1000);
//        sprintf(data, "%d", result);
        M5.Lcd.drawCentreString(data, 0, 20, 7);
